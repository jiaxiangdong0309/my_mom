...
在使用 AI 编辑器的过程中，我们常常希望 AI 能够记住之前的操作和对话，以便在后续对话中复用这些知识。典型场景包括：

- **问题解决**：我之前是怎么解决这个问题的？
- **技术调研**：我之前是怎么调研这个技术的？
- **代码片段**：我之前是怎么写这个代码的？

目前 Cursor 和 Claude 都提供了记忆功能，但存在以下局限性：

- **上下文膨胀**：基于文本文件的记忆（如 `rules`、`claude.md`、`agent.md` 等）直接插入上下文，导致上下文过长，模型注意力分散
- **搜索能力弱**：无法进行语义搜索，只能通过关键词匹配
- **使用成本高**：需要手动整理和存储记忆，操作繁琐

因此，我们实现了一个**跨会话、跨应用**的知识记忆工具来解决这些问题。
