...
1. **禁止启动服务** - 永远不要执行 `python3 main.py`、`npm run dev`、`mymem start` 等启动命令。提示用户手动运行。
...
- 💡 **快速创建**：随机建议辅助快速录入
- 📊 **用户画像**：标签统计可视化（词云、饼图、柱状图）
- 🚀 **CLI 工具**：`mymem` 命令行工具，支持前台/后台启动
- 📦 **打包安装**：支持 pip 安装，一键部署
...
2. **极简架构**：避免过度设计，够用即可
3. **单用户场景**：当前版本面向单用户
4. **不自动运行服务**：提示用户手动启动（`mymem start` 或 `python3 scripts/run_dev.py`）
5. **智能路径选择**：
   - 开发环境（存在 `.git`）：`./data/`
   - 用户环境：`~/.mymem/data/`
   - 可通过 `MYMEM_DATA_PATH` 环境变量自定义
...
### CLI 工具
- **命令**: `mymem`（通过 `pip install -e .` 安装后可用）
...
5. **数据位置**：开发环境使用 `./data/`，用户环境使用 `~/.mymem/data/`，可通过 `MYMEM_DATA_PATH` 自定义
