...
3. **数据库路径**
   - 脚本默认连接到 `backend/data/memories.db`（相对于项目根目录）
   - 如果默认路径不存在，会自动尝试相对于脚本位置的路径
   - 如需指定其他路径：`python3 .claude/skills/query-knowledge-base/search/sqlite-search/query_db.py --db /path/to/db <command>`
   - 数据库不存在时会返回 JSON 格式的错误信息
