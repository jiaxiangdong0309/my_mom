...
**前置条件**：
- 后端服务必须已启动（运行 `python3 backend/main.py`）
...
| `--api-url` | API 接口地址（可选，默认：`http://localhost:8000/api/v1/memories/`） | `--api-url http://localhost:8000/api/v1/memories/` |
...
- 使用前请确保后端服务已启动（运行 `python3 backend/main.py`），否则会提示连接错误
...
**解决方法**:
1. 确保后端服务已启动（运行 `python3 backend/main.py`）
2. 检查 API 地址是否正确（默认：`http://localhost:8000/api/v1/memories/`）
...
```json
{
  ...
  "error": "无法连接到 API 服务器: http://localhost:8000/api/v1/memories/\n请确保后端服务已启动（运行 python3 backend/main.py）"
}
```
...
| 无标签 | 省略 `--tags` 参数 |
| 后端服务 | 使用前确保后端已启动（`python3 backend/main.py`） |
